...
You will need the smart contracts we wrote in part 1, and the deployments you made in part 2. Therefore, for this last part to work you will need to have completed the prior two tutorials. 


# Setup 

The first step is to create a new NodeJS file to write our smart contract interactions in. In the root of your project, create a file named ``interact.js``.  

We will use the packages we installed via NPM in the first tutorial here. We will only need one additional module, called BigNumber, in order to work with the large numbers the Celo blockchain uses. 
...
The first step for interacting with our smart contracts is importing the modules we'll need to write our script. 
...
The next thing we want to go is get our smart contract. We're going to be reading and writing to the smart contract, so we'll need to sign in to our Celo account. Reading from smart contracts doesn't require a transaction fee, but writing does. 
...
After all the imports, write the above two lines in order to connect ``web3`` and Celo's ``contractKit`` to your Celo account. The above code imports your private key from the ``.env`` file and puts it into the account variable, as well as contractKit. 

The next step is to create an async function, where we will include all our interactions with the smart contract. 
...
Next, comment out the ``console.log`` of the ``celoCrowdfundContract`` variable. We're going to create a project in our crowdfunding contract. 

Create a new function outside of ``interact()`` called ``createProject``: 

```
async function createProject(celoCrowdfundContract, stableToken)  {
  var projectGoal = BigNumber(1E18);
  await celoCrowdfundContract.methods.startProject(stableToken.address, 'Test project', 'We are testing the create project function', 'https://i.imgur.com/T9RAp1T.jpg',  5, projectGoal).send({from: account.address, feeCurrency: stableToken.address});
  console.log("Created new project");
}
```

This function takes in the ``celoCrowdfunding`` contract, a ``stableToken`` variable. It then creates a ``projectGoal`` using ``BigNumber``. 
...
These are the parameters we supply when we call the function. Now that we have that, let's call our helper function inside ``interact()``: 
...
We use the ``stableToken`` variable in order to get the cUSD wrapper from ``contractKit`` and pass it into ``createProject``. 

We also return the list of project addresses by calling the ``returnProjects()`` function in our solidity contract. 
...
This will create a variable to access the first Project created in the list of projects returned by the ``returnProjects()`` function earlier. 

Now that we can access our project, we'll need to do two things in order to send cUSD to the contract. Since cUSD follows the [ERC-20](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/) standard, we'll need to approve sending cUSD to the contract prior to sending money. The second step is actually sending the cUSD. 
...
After we approve the contract to receive 5 cUSD from our wallet, we will need to wait a couple of seconds to see the change on the blockchain before we send some cUSD. 
...
Next, just use it to wait 5 seconds in our ``interact()`` function:
...
Great! Now our script will approve our contract to receive 5 cUSD. The next step is to actually send some money. 
...
Inside the function, we call the ``contribute()`` function in our ``Project()`` solidity contract and send it 2 cUSD. 

Back in our ``interact()`` function, let's call this new function we created: 
...
Next, inside the ``interact()`` function just call the ``printBalances()`` function: 
...
## Paying out our Project contract
...
And finally, let's call this function inside our ``interact()`` function: 
